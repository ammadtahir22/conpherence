    ...
    <div id="main" role="main">
        ...
        <div id="content">
            ...
            <section id="widget-grid" class="">
                <!-- row -->
                <div class="row">
                    <form action="{{route('admin.getuser.salereport')}}" method="post" id="earning_report_form">
                    @csrf
                    <!-- NEW WIDGET START -->
                                    ...
                                    <div class="form-horizontal">
                                        <fieldset>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-10">
                                                    <select class="form-control select2" name="user_category" id="user_category">
                                    ...
                                    <div class="widget-body no-padding">
                                        <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                                            ...
                                            @foreach($sales as $key => $sale)
                                                <tr>
                                                    <td>{{$key+1}}</td>
                                                    <td>{{$sale->booking_number}}</td>
                                                    <td>{{$sale->booking_firstname." ".$sale->booking_lastname}}</td>
                                                    <td>{{$sale->booking_email}}</td>
                                                    <td>{{user_batch_category($sale->user_id)}}</td>
                                                    <td>{{$sale->booking_phone}}</td>
                                                    <td>{{$sale->booking_address}}</td>
                                                    <td>{{get_venue_title($sale->venue_id)}} ({{get_city_by_venue($sale->venue_id)}})</td>
                                                    <td>{{get_space_title($sale->space_id)}}</td>
                                                    {{--<td>{{date('d M Y', strtotime($sale->start_date))}} to {{date('d M Y', strtotime($sale->end_date))}}</td>--}}
                                                    <td>AED   {{number_format($sale->grand_total, 2)}}/-</td>
                                                </tr>
                                                @php $total_amount = $total_amount + $sale->grand_total; @endphp
                                            @endforeach
                                            </tbody>
                                            <tfoot>
                                            <tr>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#configreset').click(function(){
                $('#earning_report_form')[0].reset();
                $('#hotel').val('').trigger("change");
