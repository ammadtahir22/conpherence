...
@section('content')
    ...
    <section class="blog-banner">
        <div id="newsslider" class="carousel slide blogslider" data-ride="carousel">
            <ol class="carousel-indicators">
                @php $count = 0; @endphp
                @if(count($news) > 0)
                    @foreach($news as $kay=>$new)
                        @if(isset($new->image))
                            @if($count < 3)
                                    <li data-target="#newsslider" data-slide-to="{{$count}}" class="{{$count==0 ? 'active' : ''}}"></li>
                            @endif
                            @php $count++; @endphp
                        @endif
                    @endforeach
            ...
            <div class="carousel-inner">
                ...
                @if(count($news) > 0)
                    @foreach($news as $kay=>$new)
                        @if(isset($new->image))
                            @if($count < 3)
                            <div class="item {{$count==0 ? 'active' : ''}}" style="background-image: url({{'storage/images/news/'.$new->id.'/'.$new->image}});">
                                <div class="container">
                                    <div class="banner_info">
                                        <h1>{{$new->title}}</h1>
                                        <img src="{{url('images/bar.png')}}" alt="" />
                                        <div class="full-col bloq-banner-btm">
                                            <div class="date">{{ date('d M Y', strtotime($new->created_at)) }}</div>
                                            <a href="{{url('news/'. $new->id)}}" class="btn get-btn"><span>View Detail</span><span></span><span></span><span></span><span></span> </a>
                                        </div>
                                    </div>
                                </div><!-- container -->
                            </div>
                    ...
                    @if(count($news) > 0)
                        @foreach($news as $kay=>$new)
                            <div class="boq-box col-sm-4">
                                @if(isset($new->image))
                                    <div class="boq-box-img"><img src="{{url('storage/images/news/'.$new->id.'/'.$new->image)}}"></div>
                                @else
                                    <div class="boq-box-img"><img src="{{url('images/placeholder.jpg')}}"></div>
                                @endif
                                <div class="boq-box-info">
                                    <a href="{{url('news/'. $new->id)}}"> <h3>{{ substr($new->title,0,30) }} </h3> </a>
                                    <p> {{ substr(strip_tags($new->description),0,100) }} <a href="{{url('news/'. $new->id)}}">...</a></p>
                                </div>
                            </div><!-- boq-box -->
                        @endforeach
                    @else
                        ...
                        @foreach($news as $kay=>$new)
                            @if($kay < 6)
                                <div class="sidebar-boq">
                                    @if(isset($new->image))
                                        <div class="sidebar-boq-img"><img src="{{url('storage/images/news/'.$new->id.'/'.$new->image)}}"></div>
                                    @else
                                        <div class="sidebar-boq-img"><img src="{{url('images/placeholder.jpg')}}"></div>
                                    @endif
                                    <div class="sidebar-boq-info">
                                        <a href="{{url('news/'. $new->id)}}"> <h3>{{ substr($new->title,0,20) }} </h3> </a>
                                        <p> {{ substr(strip_tags($new->description),0,50) }} <a href="{{url('news/'. $new->id)}}">...</a></p>
                                    </div>
                                </div><!-- boq-box -->
                            @endif
                        @endforeach
